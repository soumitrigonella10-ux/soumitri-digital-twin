...
export default function SkinPage() {
  ...
  const skinProducts = useMemo(() => {
    return data.products
      .filter((p) => p.routineType === "skin")
      .sort((a, b) => (a.displayOrder || 999) - (b.displayOrder || 999));
  }, [data.products]);

  // Separate products by time of day and apply day filter
  const morningProducts = useMemo(() => {
    let filtered = skinProducts.filter(p => p.timeOfDay === "AM" || p.timeOfDay === "ANY");
    ...
    return filtered;
  }, [skinProducts, activeDayFilter]);

  const eveningProducts = useMemo(() => {
    let filtered = skinProducts.filter(p => p.timeOfDay === "PM" || p.timeOfDay === "ANY");
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500"
                  />
                </div>
