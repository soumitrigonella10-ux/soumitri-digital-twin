...
import { Moon, Clock, Sparkles, Edit2, Trash2, Plus } from "lucide-react";
...
/* ─── Meal Template Form Modal (Dinner) ─── */
function MealFormModal({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const payload = {
        id: meal?.id ?? `mt-dinner-${name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`,
        name: name.trim(),
        timeOfDay: "PM",
        mealType: "dinner",
  ...
  }, [name, prepTimeMin, cookTimeMin, servings, weekdays, items, ingredients, instructions, tags, meal?.id, onSaved, onClose]);
  ...
  return (
    ...
    >
      <Field label="Name *">
        <input value={name} onChange={(e) => setName(e.target.value)} className={inputClass} placeholder="e.g. Grilled Salmon Bowl" />
      </Field>
      ...
      <Field label="Ingredients">
        ...
        <button type="button" onClick={() => setIngredients([...ingredients, { name: "", quantity: "", unit: "" }])} className="mt-1 text-xs text-indigo-600 hover:text-indigo-700 flex items-center gap-1"><Plus className="w-3 h-3" /> Add ingredient</button>
      </Field>
      <Field label="Instructions">
        ...
        <button type="button" onClick={() => setInstructions([...instructions, ""])} className="mt-1 text-xs text-indigo-600 hover:text-indigo-700 flex items-center gap-1"><Plus className="w-3 h-3" /> Add step</button>
      </Field>
      <Field label="Tags (comma-separated)">
        <input value={tags} onChange={(e) => setTags(e.target.value)} className={inputClass} placeholder="e.g. light, protein-rich" />
      </Field>
    </AdminCrudModal>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="animate-fade-scale">
        ...
        {isAdmin && (
          <AdminAddButton onClick={() => setShowAddModal(true)} label="Add Dish" accentColor="bg-indigo-500" />
        )}
      </header>
              ...
              <div className="flex items-start justify-between">
                ...
                {isAdmin && (
                  <div className="flex items-center gap-1 ml-2">
                    <button onClick={() => setEditingMeal(meal)} className="p-1.5 text-gray-400 hover:text-indigo-600 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
      ...
      {/* CRUD Modals */}
      {showAddModal && (
        <MealFormModal onClose={() => setShowAddModal(false)} onSaved={refreshFromDb} />
      )}
      {editingMeal && (
        <MealFormModal meal={editingMeal} onClose={() => setEditingMeal(null)} onSaved={refreshFromDb} />
      )}
      {deletingMeal && (
        <DeleteConfirmModal
