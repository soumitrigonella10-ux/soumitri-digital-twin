...
export default function InnerwearPage() {
  ...
  // Filter for innerwear only
  const innerwearItems = useMemo(() => {
    return data.wardrobe.filter((item) => item.category === "Innerwear" || item.category === "Intimates");
  }, [data.wardrobe]);

  // Filter items
  const filteredItems = useMemo(() => {
    let items = [...innerwearItems];
    ...
    return items;
  }, [innerwearItems, selectedType, selectedSubType]);
