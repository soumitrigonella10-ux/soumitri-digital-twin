...
function ActivewearPageContent() {
  const { data } = useAppStore();

  const activewearItems = useMemo(() => {
    return data.wardrobe.filter((item) => item.category === "Activewear");
  }, [data.wardrobe]);
      ...
      <header className="animate-fade-scale">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Activewear</h1>
              <p className="text-gray-500">{activewearItems.length} items</p>
            </div>
          </div>
        </div>
      </header>
