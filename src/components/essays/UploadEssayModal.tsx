...
// ─────────────────────────────────────────────────────────────
// Upload New Essay Modal — Admin-only inline essay creation
//
// Matches the editorial design system of the essays page.
...
export function UploadEssayModal({ onClose, onPublished }: UploadEssayModalProps) {
    ...
    try {
      const result = await createContent({
        ...
        metadata: {
          category,
          tags: [],
          date: publishDate || new Date().toLocaleDateString("en-US", { month: "long", year: "numeric" }),
        },
        payload: {
      ...
      if (result.success) {
        ...
        setError(result.error || "Failed to publish essay");
      }
    } catch (err) {
  ...
  }, [title, category, publishDate, readingTime, excerpt, pdfUrl, onClose, onPublished]);
      ...
      <div className="relative bg-[#FFF8F0] rounded-xl shadow-2xl w-full max-w-xl mx-4 max-h-[85vh] overflow-y-auto border border-stone-200">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="A brief hook for the homepage..."
              rows={3}
              maxLength={500}
              ...
              {isUploading ? (
                ...
                <>
                  ...
                  <span className="text-xs text-stone-400 font-editorial mt-1">
                    {(pdfFile.size / 1024 / 1024).toFixed(1)} MB · Click to replace
                  </span>
                </>
              ) : (
                <>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-stone-200">
          ...
          <button
            ...
            disabled={isSubmitting || isUploading || !title.trim() || !excerpt.trim()}
