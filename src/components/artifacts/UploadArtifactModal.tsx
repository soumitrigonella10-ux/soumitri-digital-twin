...
// ─────────────────────────────────────────────────────────────
// Upload New Artifact Modal — Admin-only inline artifact creation
//
// Matches the editorial design system.
// Submits directly to CMS server actions (createContent).
//
// Artifact parameters:
//   title, medium, date, dimensions, frameType, borderStyle,
//   backgroundColor, description, imagePath, paperNote, hasWashiTape
// ─────────────────────────────────────────────────────────────
...
export function UploadArtifactModal({ onClose, onPublished }: UploadArtifactModalProps) {
  // Base fields
  ...
  const [borderStyle, setBorderStyle] = useState("shadow");
  const [backgroundColor, setBackgroundColor] = useState("#F9F7F2");
  const [hasWashiTape, setHasWashiTape] = useState(false);

  // Paper note
  const [paperNoteText, setPaperNoteText] = useState("");
  const [paperNotePosition, setPaperNotePosition] = useState("");
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || "Failed to publish artifact");
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[3vh] sm:pt-[5vh]">
      ...
      <div className="relative bg-[#FFF8F0] rounded-xl shadow-2xl w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto border border-stone-200">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {/* Description (for wide/text cards) */}
          <div>
            <label className="block font-editorial text-[10px] font-semibold uppercase tracking-[0.15em] text-telugu-marigold mb-1.5">
              Description
          ...
          <div>
            ...
            >
              ...
              {isUploading ? (
                <>
                  <Loader2 className="h-8 w-8 text-stone-400 animate-spin mb-2" />
                ...
                <>
                  ...
                  <span className="text-xs text-stone-400 font-editorial mt-1">
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-stone-200">
          ...
          <button
            ...
            disabled={isSubmitting || isUploading || !title.trim() || !medium.trim()}
