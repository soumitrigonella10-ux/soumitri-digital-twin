...
import type { LoreCategory, LoreMediaType } from "@/data/internetLore";

interface UploadLoreModalProps {
...
export function UploadLoreModal({ defaultCategory = "pop-internet-core", onClose, onSuccess }: UploadLoreModalProps) {
  const [title, setTitle] = useState("");
  const [category, setCategory] = useState<LoreCategory>(defaultCategory);
  const [mediaType, setMediaType] = useState<LoreMediaType>("photo");
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    if (!validExts.includes(ext)) {
      setError("Only image files (jpg, png, webp, avif, gif) are accepted");
      return;
    }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (mediaType === "quote" && !quoteText.trim()) { setError("Quote text is required"); return; }
    if (mediaType === "video" && !videoUrl.trim()) { setError("Video URL is required"); return; }
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[5vh] sm:pt-[8vh]">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-xl mx-4 max-h-[85vh] overflow-y-auto border border-gray-200">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Add Lore</h2>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Rickrolling" className="w-full px-3 py-2 rounded-lg border border-gray-300 bg-white text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#802626]/40 focus:border-[#802626]" />
          </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center gap-0.5 px-1.5 py-2 rounded-lg border text-[9px] font-medium uppercase tracking-wider transition-all ${
                        mediaType === opt.value
                          ? "border-[#802626] bg-[#802626]/5 text-[#802626]"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={era} onChange={(e) => setEra(e.target.value)} placeholder="e.g. 2015, Pre-Internet" className="w-full px-3 py-2 rounded-lg border border-gray-300 bg-white text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#802626]/40 focus:border-[#802626]" />
            </div>
            <div>
              ...
              >
                {isUploading ? (
                  <div className="flex flex-col items-center gap-2">
                    <Loader2 className="h-7 w-7 animate-spin text-[#802626]" />
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-xs text-gray-400 truncate max-w-full">{imageFile?.name || mediaUrl}</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-2">
                    <Upload className="h-7 w-7 text-gray-400" />
                    <p className="text-sm text-gray-500">Drop an image or click to browse</p>
          ...
          {showVideoUrl && (
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1.5">
                {mediaType === "reel" ? "Reel URL" : "Video URL"}
              </label>
              <input type="url" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} placeholder="https://youtube.com/watch?v=... or https://instagram.com/reel/..." className="w-full px-3 py-2 rounded-lg border border-gray-300 bg-white text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#802626]/40 focus:border-[#802626]" />
            </div>
          )}
          ...
          <div>
            ...
            <input type="text" value={note} onChange={(e) => setNote(e.target.value)} placeholder="A scribble shown on hover..." className="w-full px-3 py-2 rounded-lg border border-gray-300 bg-white text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#802626]/40 focus:border-[#802626]" />
          </div>
          ...
          {success && (
            <div className="flex items-center gap-2 text-sm text-green-700 bg-green-50 px-3 py-2 rounded-lg">
              <Check className="h-4 w-4" /> Lore added successfully!
            </div>
          )}
        </div>
