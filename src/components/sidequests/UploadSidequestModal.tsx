...
interface UploadSidequestModalProps {
  onClose: () => void;
  onSuccess?: () => void;
}
...
export function UploadSidequestModal({ onClose, onSuccess }: UploadSidequestModalProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || "Failed to create sidequest");
      }
    } catch (err) {
  ...
  }, [title, category, description, questLog, imageUrl, onClose, onSuccess]);
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[5vh] sm:pt-[8vh]">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-xl mx-4 max-h-[85vh] overflow-y-auto border border-gray-200">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">New Sidequest</h2>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {success && (
            <div className="flex items-center gap-2 text-sm text-green-700 bg-green-50 px-3 py-2 rounded-lg">
              <Check className="h-4 w-4" /> Sidequest created successfully!
            </div>
          )}
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
          ...
          <button onClick={handleSubmit} disabled={isSubmitting || success} className="px-5 py-2 text-sm rounded-lg bg-[#802626] text-white hover:bg-[#6b1f1f] disabled:opacity-50 flex items-center gap-2">
            ...
            {isSubmitting ? "Creating..." : "Create Sidequest"}
          </button>
        </div>
      </div>
    </div>
