...
export function EditSkillModal({ skill, onClose, onSuccess }: EditSkillModalProps) {
  const [name, setName] = useState(skill.name);
  const [field, setField] = useState<string>(skill.category);
  const [keywords, setKeywords] = useState(skill.tools.join(", "));
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || "Failed to update skill quest");
      }
    } catch (err) {
  ...
  }, [name, field, keywords, proficiency, description, skill.id, onClose, onSuccess]);
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[5vh] sm:pt-[8vh]">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-xl mx-4 max-h-[85vh] overflow-y-auto border border-gray-200">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Edit Skill Quest</h2>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {success && (
            <div className="flex items-center gap-2 text-sm text-green-700 bg-green-50 px-3 py-2 rounded-lg">
              <Check className="h-4 w-4" /> Skill quest updated successfully!
            </div>
          )}
        </div>
